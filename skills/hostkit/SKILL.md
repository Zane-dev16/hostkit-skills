...
- `GET $BASE/{endpoint}?APIKEY=$HOSTKIT_API_KEY&...` URL-encoded. Log endpoint + timestamp + HTTP status + `error` text + retry count; never log full URLs, keys, guest docs, or payloads.
